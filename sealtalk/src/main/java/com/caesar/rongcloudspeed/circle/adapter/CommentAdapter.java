...
public class CommentAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.circleMovementMethod.setContext(mContext);
...
//        }
    ...
    private SpannableString setClickableSpan(String textStr, int position) {
        ...
        subjectSpanText.setSpan(new NameClickable(new NameClickListener(subjectSpanText, "") {
                    @Override
                    public void onSpanClick(int position) {
                        Toast.makeText(mContext, subjectSpanText + " &id = " + "", Toast.LENGTH_SHORT).show();
                    }
                }, position,mContext.getResources()), 0, subjectSpanText.length(),
