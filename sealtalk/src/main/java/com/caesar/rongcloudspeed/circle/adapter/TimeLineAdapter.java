 ...
 */
public class TimeLineAdapter extends BaseAdapter implements ICircleViewUpdate {
    ...
    private static final String ITEM_TYPE_IMTERM = "41";
    private static final String ITEM_TYPE_URL = "2";
    private static final String ITEM_TYPE_VIDEO = "3";
    private static final String ITEM_TYPE_VIDTERM = "44";
    private static final String ITEM_TYPE_ADVERT = "4";
    private static final String ITEM_TYPE_ADTERM = "42";
    ...
    public int getItemViewType(int position) {
        ...
        } else if (ITEM_TYPE_IMTERM.equals(item.getTerm_id())) {
            itemType = ITEM_VIEW_TYPE_IMAGE;
        } else if (ITEM_TYPE_VIDTERM.equals(item.getTerm_id())) {
            itemType = ITEM_VIEW_TYPE_VIDEO;
        } else if (ITEM_TYPE_ADTERM.equals(item.getTerm_id())) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.deleteBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                //删除
                builder = new AlertDialog.Builder(mContext).setIcon(R.mipmap.ic_launcher).setTitle("提示")
                        .setMessage("您确定进行删除操作吗？").setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            @Override
            ...
            if (hasComment) {//处理评论列表
                ...
                holder.bbsAdapter.setCommentClickListener(new ICommentItemClickListener() {
                    ...
                    public void onItemClick(int commentPosition) {
                        ...
                        ((FriendTimeLineActivity) mContext).showCmmtPop(position);
                    }
                });
...
//                holder.urlTipTv.setVisibility(View.VISIBLE);
        ...
        public void onItemClick(ActionItem actionitem, int position) {
            switch (position) {
                case 0://点赞、取消点赞
                    ...
                    break;
                case 1://发布评论
                    ((FriendTimeLineActivity) mContext).showCmmtPop(mCirclePosition);
                    break;
                default:
                    break;
            }
    ...
    public void updateAddComment(int circlePosition, String username, String content) {
        CommentItem newItem = DatasUtil.createPublicComment(content, username);
        getDatas().get(circlePosition).getLast_comments().add(newItem);
        notifyDataSetChanged();
        updateComment(getDatas().get(circlePosition).getObject_id(), content);
    }
